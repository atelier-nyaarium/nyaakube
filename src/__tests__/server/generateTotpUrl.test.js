...
describe("generateTotpUrl", () => {
	it(`should generate a TOTP URL`, () => {
		expect(
			generateTotpUrl("Atelier Nyaarium", "user@email", 6, 30, 30, "ABC"),
		).toEqual(
	...
	it(`should throw error if label is not a string`, () => {
		expect(() => generateTotpUrl(123, 6, 30, 30, "ABC")).toThrow(TypeError);
	});
	...
	it(`should throw error if step is not a number`, () => {
		expect(() =>
			generateTotpUrl("Foo Bar:user@email", 6, 30, "30", "ABC"),
		).toThrow(TypeError);
	});
	...
	it(`should throw error if secret is not a string`, () => {
		expect(() =>
			generateTotpUrl("Foo Bar:user@email", 6, 30, 30, 123),
		).toThrow(TypeError);
	});
});
